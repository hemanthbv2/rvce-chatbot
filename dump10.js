...
let cleanInput = 'highest package in cse';
...
if (pMatch) {
    ...
    extractedDept = extractedDept.replace(/\b(engineering|technology|department|dept)\b/gi, '').trim().toLowerCase();
    const KB = global.test_KB;
    console.log("extractedDept:", extractedDept);
    
    let m1 = KB.departments.ug.find(x => x.c === extractedDept);
    let m2 = KB.departments.pg.find(x => x.c === extractedDept);
    let m3 = KB.departments.ug.find(x => x.n.toLowerCase().startsWith(extractedDept));
    let m4 = KB.departments.pg.find(x => x.n.toLowerCase().startsWith(extractedDept));
    let m5 = KB.departments.ug.find(x => x.n.toLowerCase().includes(extractedDept));
    let m6 = KB.departments.pg.find(x => x.n.toLowerCase().includes(extractedDept));
