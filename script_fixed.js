// 6. Placement Regex
    let deptName = null;
    let pMatch = /(?:placement|placements|highest package|average package|salary)\s+(?:for|in|of)?\s*([a-zA-Z\s\(\)]+)/i.exec(cleaned);
    if (!pMatch) pMatch = /([a-zA-Z\s\(\)]+)\s+(?:placement|placements|highest package|average package|salary)/i.exec(cleaned);
    ...
    if (pMatch) {
        ...
        if (matched) {
            return { intent: 'plcmt_' + matched.c, year: yearMatch ? yearMatch[1] : null };
        }
    }
                ...
                if (rawStats) {
                    if (!isUG && !isPG && (rawStats.ug || rawStats.pg)) {
                        ...
                        if (typeof SESSION !== 'undefined' && SESSION.reqYear) {
                            // If they asked for a year, but didn't specify UG or PG, default to UG if available, else PG
                    ...
                    if (typeof SESSION !== 'undefined' && SESSION.reqYear) {
                        ...
                        if (stats.full) {
                            specificProg = stats.full.find(p => p.name && p.name.includes(yr));
                        }
                        // Also check legacy programs array
                        if (!specificProg && stats.programs) {
                            specificProg = stats.programs.find(p => p.name && p.name.includes(yr));
                        }
                        if (!specificProg && stats.ongoing && (stats.ongoing.name||'').includes(yr)) {
                            specificProg = stats.ongoing;
                        }
                        
                        if (!specificProg && Array.isArray(stats.ongoing)) {
                            specificProg = stats.ongoing.find(p => p.name && p.name.includes(yr));
                        }
                        
                        if (specificProg) {
                            r.text += T(`Here are the placement highlights for **${d.n}** for the year ${yr}: 🚀\n\n`, `Detailed Placement Statistics for ${d.n} (${yr}):\n\n`);
                            ...
                            r.text += `I couldn't find specific placement statistics for **${SESSION.reqYear}** in **${d.n}**. Here is the most recent data instead:\n\n`;
                        }
                    } else {
                    ...
                    if (stats.ongoing) {
                        ...
                        // No valid format found inside stats
