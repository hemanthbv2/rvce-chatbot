...
(async () => {
    console.log("Crawling ECE faculty page (https://rvce.edu.in/department/ece/faculty/)...");
    const browser = await puppeteer.launch({ headless: true, args: ['--no-sandbox'] });
    const page = await browser.newPage();
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36');
    await page.setViewport({ width: 1366, height: 768 });

    await page.goto('https://rvce.edu.in/department/ece/faculty/', { waitUntil: 'networkidle2', timeout: 60000 });
    ...
    const facultyMembers = await page.evaluate(() => {
        ...
        links.forEach(a => {
            ...
            if (href.includes('/department/ece/') || href.includes('/department/electronics/')) {
    ...
    console.log(`Extracted ${facultyMembers.length} ECE faculty members with direct profile URLs.`);
    console.log(JSON.stringify(facultyMembers, null, 2));

    await browser.close();

    fs.writeFileSync('ece_scraped.json', JSON.stringify(facultyMembers, null, 2));
})();
