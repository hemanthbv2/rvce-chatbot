...
(async () => {
    console.log("Crawling EEE faculty page for exact faculty profiles...");
    ...
    const facultyMembers = await page.evaluate(() => {
        const results = [];
        // Extract links pointing to department profiles or containing faculty name headings
        const links = document.querySelectorAll('a[href*="/department/"]');
        links.forEach(a => {
            const href = a.href;
            const text = a.innerText.trim();
            if (href.includes('/department/eee/') || href.includes('/department/electrical/')) {
                if (text && !text.includes('Department') && !text.includes('Faculty') && !text.includes('Read More') && text.length < 50) {
                    if (!results.some(r => r.url === href)) {
                        // Find parent container text for designation
    ...
    console.log(`Extracted ${facultyMembers.length} EEE faculty members with direct profile URLs.`);
    console.log(JSON.stringify(facultyMembers, null, 2));

    await browser.close();

    fs.writeFileSync('eee_scraped_v2.json', JSON.stringify(facultyMembers, null, 2));
})();
